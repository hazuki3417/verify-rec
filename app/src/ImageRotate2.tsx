...
async function getBitmapOrImageFromSrc(src: string): Promise<ImageBitmap | HTMLImageElement> {
...
export default function ImageRotateAndDownloadFromSrc() {
  ...
  const onDownload = async () => {
    ...
    setBusy(true);
    try {
      const { blob, suggestedName } = await buildRotatedImageBlobFromSrc(src, deg, 0.92, "#fff");
    ...
    <div style={{ display: "grid", gap: 8, maxWidth: 520 }}>
      ...
      {preview && (
        <img
          src={preview}
          alt="preview"
          style={{ maxWidth: "100%", maxHeight: 240, objectFit: "contain", border: "1px solid #ddd" }}
          crossOrigin="anonymous"
        />
      )}

      <div style={{ display: "flex", gap: 8 }}>
        <button type="button" onClick={rotateLeft} disabled={!src || busy}>左に90°</button>

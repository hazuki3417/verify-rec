...
const Base = styled.p`
  padding: 0px;
  margin: 0px;
  color: var(--color-base-river-blue);
...
アプリケーション固有のカラートークンがあり、それらは css の変数として定義されています。
css の変数を利用せずに直接色指定をした場合、カラートークンから外れた色指定となるため変数を使用しない色指定は避けてください。
